...
"&& OPT=\"\$CFLAGS -DTHREAD_STACK_SIZE=0x100000\" make -s EXTRA_CFLAGS=\"\$CFLAGS -DTHREAD_STACK_SIZE=0x100000\" PROFILE_TASK=\"-m test.regrtest --pgo test_array test_base64 test_binascii test_binhex test_binop test_bytes test_c_locale_coercion test_class test_cmath test_codecs test_compile test_complex test_csv test_decimal test_dict test_float test_fstring test_hashlib test_io test_iter test_json test_long test_math test_memoryview test_pickle test_re test_set test_slice test_struct test_time test_traceback test_unicode test_threading \" "\
'&& make install '\
